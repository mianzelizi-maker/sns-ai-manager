...
AIを活用してWordPress記事をSNS(X / Instagram)向けに自動展開する、SNS運用効率化ツールです。公開されている実際の業務委託案件を参考に、要件を独自に定義してポートフォリオとして開発しました。
...
- **即時投稿・予約投稿**: 承認済みの投稿をX/Instagramへ即時実行、または日時を指定して予約実行。添付する画像は「AI生成画像」「記事に掲載されている画像」のどちらを使うか選択可能
...
- **閲覧専用モード**: `READ_ONLY_MODE=true`で全ての更新系操作(投稿・承認・生成など)を無効化。誰でもアクセスできる公開デモ環境向け
...
READ_ONLY_MODE=                  # trueにすると全ての更新系操作を無効化(公開デモ用)
```
